 ...
 *
 * Note: AGP 9.0 has significant breaking changes:
 * - New DSL interfaces (android.newDsl=true by default)
 * - Legacy Variant API disabled by default
 * - Built-in Kotlin support enabled by default
 * - Some internal APIs may have changed
 *
 * This implementation may need adjustments based on actual AGP 9.0 API changes.
 */
...
internal object V90 : AGPInterface {
    ...
    override val Variant.rawAndroidResources: FileCollection
        get() {
            ...
            // MICRO_APK_RES may be removed in AGP 9.0
            try {
                val microApkResType = InternalArtifactType::class.sealedSubclasses
                    .mapNotNull { it.objectInstance }
                    .filterIsInstance<Artifact.Single<*>>()
                    .firstOrNull { it.javaClass.simpleName == "MICRO_APK_RES" }
                if (microApkResType != null) {
                    component.taskContainer.generateApkDataTask?.let {
                        @Suppress("UNCHECKED_CAST")
                        allRes.from(artifactsImpl.get(microApkResType as Artifact.Single<out FileSystemLocation>))
                    }
                }
            } catch (_: Throwable) {
                // Ignore if MICRO_APK_RES is not available
            }
                            ...
                            directoryEntries.directoryEntries
                                    .mapNotNull {
                                        if (it is TaskProviderBasedDirectoryEntryImpl) {
                                            ...
                                            // Use reflection to get file tree in AGP 9.0
                                            try {
                                                val method = it.javaClass.getMethod("asFileTree", java.util.function.Function::class.java)
                                                method.invoke(it, java.util.function.Function<Any, Any> { dir ->
                                                    component.services.fileCollection().from(dir)
                                                })
                                            } catch (_: Throwable) {
                                                try {
                                                    // Fallback: try getAsFileTree
                                                    it.javaClass.getMethod("getAsFileTree").invoke(it)
                                                } catch (_: Throwable) {
                                                    null
                                                }
                                            }
    ...
    override val Variant.sourceSetMap: FileCollection
        get() = try {
            // SOURCE_SET_PATH_MAP may be removed in AGP 9.0
            val sourceSetPathMapType = InternalArtifactType::class.sealedSubclasses
                .mapNotNull { it.objectInstance }
                .filterIsInstance<Artifact.Single<*>>()
                .firstOrNull { it.javaClass.simpleName == "SOURCE_SET_PATH_MAP" }
            if (sourceSetPathMapType != null) {
                @Suppress("UNCHECKED_CAST")
                getFinalArtifactFiles(sourceSetPathMapType as Artifact.Single<out FileSystemLocation>)
            } else {
                project.files()
            }
        } catch (_: Throwable) {
    ...
    override val Variant.localAndroidResources: FileCollection
        get() {
            ...
            try {
                // Simplified implementation for AGP 9.0
                // Just return an empty collection with task dependency on merge resources
                fileCollection.builtBy(mergeResourcesTaskProvider)
            } catch (_: Throwable) {
                // Fallback for AGP 9.0 API changes
            }
    ...
    override val Variant.targetVersion: AndroidVersion
        get() = try {
            // targetSdkVersion is deprecated/error in AGP 9.0
            // Use reflection to access the property
            val method = this.javaClass.getMethod("getTargetSdkVersion")
            method.invoke(this) as AndroidVersion
        } catch (_: Throwable) {
            // Fallback to component's targetSdk or minSdk
            try {
                component.javaClass.getMethod("getTargetSdk").invoke(component) as AndroidVersion
            } catch (_: Throwable) {
                try {
                    // Last resort: use minSdk
                    minSdk
                } catch (_: Throwable) {
                    // Create a default AndroidVersion
    ...
    override val Variant.mergedAssets: FileCollection
        get() = when (this) {
            ...
            is LibraryVariant -> {
                // LIBRARY_ASSETS was removed in AGP 8.9+
                // Try to get it through reflection or fallback to merge assets task output
                val libraryAssetsType = try {
                    InternalArtifactType::class.sealedSubclasses
                        .mapNotNull { it.objectInstance }
                        .filterIsInstance<Artifact.Single<*>>()
                        .firstOrNull { it.javaClass.simpleName == "LIBRARY_ASSETS" }
                } catch (e: Throwable) {
                    null
                }
                if (libraryAssetsType != null) {
                    @Suppress("UNCHECKED_CAST")
                    getFinalArtifactFiles(libraryAssetsType as Artifact.Single<out FileSystemLocation>)
                } else {
                    project.files().builtBy(mergeAssetsTaskProvider)
                }
            }
    ...
    override val Variant.processedRes: FileCollection
        get() = try {
            // PROCESSED_RES was removed from InternalArtifactType in AGP 8.6
            // Try to get it through reflection for forward compatibility
            val processedResType = InternalArtifactType::class.sealedSubclasses
                .mapNotNull { it.objectInstance }
                .filterIsInstance<Artifact.Single<*>>()
                .firstOrNull { it.javaClass.simpleName == "PROCESSED_RES" }
            if (processedResType != null) {
                @Suppress("UNCHECKED_CAST")
                getFinalArtifactFiles(processedResType as Artifact.Single<out FileSystemLocation>)
            } else {
                // Fallback: return output from processResources task
                project.files().builtBy(project.tasks.named(getTaskName("process", "Resources")))
            }
        } catch (e: Throwable) {
            project.files().builtBy(project.tasks.named(getTaskName("process", "Resources")))
        }
