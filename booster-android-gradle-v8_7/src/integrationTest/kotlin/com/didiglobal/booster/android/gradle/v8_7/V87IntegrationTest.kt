...
abstract class V87IntegrationTest(private val isLib: Boolean) {
    ...
    val ruleChain: TestRule = rule(projectDir) {
        rule(LocalProperties(projectDir::getRoot)) {
            GradleExecutor(projectDir::getRoot, "8.9", *ARGS.toTypedArray())
        }
    }
    ...
    fun setup() {
        ...
        assertEquals(7, AGP.revision.minor)
    }
...
class V87AppIntegrationTest : V87IntegrationTest(false)

class V87LibIntegrationTest : V87IntegrationTest(true)
