...
abstract class V88IntegrationTest(private val isLib: Boolean) {
    ...
    val ruleChain: TestRule = rule(projectDir) {
        rule(LocalProperties(projectDir::getRoot)) {
            GradleExecutor(projectDir::getRoot, "8.10.2", *ARGS.toTypedArray())
        }
    }
    ...
    fun setup() {
        ...
        assertEquals(8, AGP.revision.minor)
    }
...
class V88AppIntegrationTest : V88IntegrationTest(false)

class V88LibIntegrationTest : V88IntegrationTest(true)
