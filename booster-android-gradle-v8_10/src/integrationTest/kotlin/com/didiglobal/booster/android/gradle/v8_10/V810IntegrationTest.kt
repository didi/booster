...
private val ARGS = System.getProperty("gradle.args").split("\\s+".toRegex()) + listOf(
    ...
    "-Pandroid_gradle_version=8.10.0",
...
abstract class V810IntegrationTest(private val isLib: Boolean) {
    ...
    fun setup() {
        ...
        assertEquals(10, AGP.revision.minor)
    }

    @Test @Case(ProjectTest::class) fun `test AGPInterface#project`() = Unit
    @Test @Case(JavaCompilerTaskTestUnit::class) fun `test AGPInterface#javaCompilerTask`() = Unit
    @Test @Case(PreBuildTaskTestUnit::class) fun `test AGPInterface#preBuildTask`() = Unit
    @Test @Case(AssembleTaskTestUnit::class) fun `test AGPInterface#assembleTask`() = Unit
    @Test @Case(MergeAssetsTaskTestUnit::class) fun `test AGPInterface#mergeAssetsTask`() = Unit
    @Test @Case(MergeResourcesTaskTestUnit::class) fun `test AGPInterface#mergeResources`() = Unit
    @Test @Case(AllClassesTestUnit::class) fun `test AGPInterface#allClasses`() = Unit
}

class V810AppIntegrationTest : V810IntegrationTest(false)
class V810LibIntegrationTest : V810IntegrationTest(true)

class ProjectTest : TestCase {
    override fun apply(project: Project) {
        project.getAndroidComponents<AndroidComponentsExtension<*, *, *>>().onVariants { variant ->
...
class JavaCompilerTaskTestUnit : VariantTestCase() {
    override fun apply(variant: Variant) {
        variant.project.gradle.taskGraph.whenReady { assertNotNull(AGP.run { variant.javaCompilerTask }) }
    }
}

class PreBuildTaskTestUnit : VariantTestCase() {
    override fun apply(variant: Variant) {
        variant.project.gradle.taskGraph.whenReady { assertNotNull(AGP.run { variant.preBuildTask }) }
    }
}

class AssembleTaskTestUnit : VariantTestCase() {
    override fun apply(variant: Variant) {
        variant.project.gradle.taskGraph.whenReady { assertNotNull(AGP.run { variant.assembleTask }) }
    }
}

class MergeAssetsTaskTestUnit : VariantTestCase() {
    override fun apply(variant: Variant) {
        variant.project.gradle.taskGraph.whenReady { assertNotNull(AGP.run { variant.mergeAssetsTask }) }
    }
}

class MergeResourcesTaskTestUnit : VariantTestCase() {
    override fun apply(variant: Variant) {
        variant.project.gradle.taskGraph.whenReady { assertNotNull(AGP.run { variant.mergeResourcesTask }) }
    }
}
        ...
        variant.project.gradle.taskGraph.whenReady {
            variant.assembleTaskProvider.configure {
                it.doFirst {
                    ...
                    assertTrue("No class file found at $location") { location.search(File::isFile).isNotEmpty() }
                }
            }
        }
    }
